...
*)
  ...
  let parse_failure pos what =
    ...
    (* TODO: Determine why pos.pos_bol fails and uncomment below *)
    (* let msg =
...
%start sexp
%type <HStringSExprPos.t> sexp

%start sexp_opt
%type <HStringSExprPos.t option> sexp_opt

%start sexps
%type <HStringSExprPos.t list> sexps

%start rev_sexps
%type <HStringSExprPos.t list> rev_sexps
...
sexp_but_no_comment
  : STRING { HStringSExprPos.Atom {pos = mk_pos $startpos ; value = $1} }
  | LPAREN RPAREN { HStringSExprPos.List [] }
  | LPAREN rev_sexps_aux RPAREN { HStringSExprPos.List (List.rev $2) }
